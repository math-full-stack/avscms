...
$requirements[0]['dp3'] = '/Applications/XAMPP/xamppfiles/bin/php';
$requirements[1]['dp3'] = '/opt/homebrew/bin/ffmpeg';
$requirements[2]['dp3'] = '/opt/homebrew/bin/ffprobe';
...
if ( isset($_POST['autofind_paths']) ) {
	foreach ($requirements as $key => $value) {
		if ( file_exists($value['path']) && is_file($value['path']) && is_executable($value['path']) )	{
			...
			} elseif ( isset($value['dp3']) && $value['dp3'] != '' && file_exists($value['dp3']) && is_file($value['dp3']) && is_executable($value['dp3']) ) {
				$config[$value['cname']] = $value['dp3'];
				$messages[] = '<b>'.$value['name'].'</b> path found: <b>'.$value['dp3'].'</b>!';
			} else {
