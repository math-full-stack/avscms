...
$def = array(
    'name' => '', 'type' => 'preroll', 'grp' => '', 'device' => 'dm', 'categories' => '-',
    'creative' => 'image', 'media_url' => '', 'code' => '', 'duration' => 0,
    'fake_progress' => '1', 'skip_after' => 0, 'schedule' => '25,50,75',
    'cap_per_hour' => 3, 'position' => 'bottom-right', 'status' => '1'
);
...
if ( isset($_POST['adv_add']) ) {
    ...
    if ( !$errors ) {
        $sql = "INSERT INTO adv_player
                (type, grp, name, device, categories, creative, media_url, code, duration,
                 fake_progress, skip_after, schedule, cap_per_hour, position, status, adv_addtime)
                VALUES (
                 " .$conn->qStr($adv['type']). ", " .$conn->qStr($adv['grp']). ", " .$conn->qStr($adv['name']). ",
                 " .$conn->qStr($adv['device']). ", " .$conn->qStr($adv['categories']). ", " .$conn->qStr($adv['creative']). ",
                 " .$conn->qStr($adv['media_url']). ", " .$conn->qStr($adv['code']). ", " .intval($adv['duration']). ",
                 " .$conn->qStr($adv['fake_progress']). ", " .intval($adv['skip_after']). ", " .$conn->qStr($adv['schedule']). ",
                 " .intval($adv['cap_per_hour']). ", " .$conn->qStr($adv['position']). ", " .$conn->qStr($adv['status']). ", " .time(). "
                )";
        $conn->execute($sql);
        $messages[] = 'Player ad successfully added!';
    }
} else {
    $adv = $def;
}
