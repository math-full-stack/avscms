...
$sql             = "SELECT count(*) AS v_downloading FROM video WHERE active='2';";
$rs              = $conn->execute($sql);
$videos['downloading'] = $rs->fields['v_downloading'];

$sql             = "SELECT count(*) AS v_queued FROM video WHERE active='3';";
$rs              = $conn->execute($sql);
$videos['queued'] = $rs->fields['v_queued'];
// Keep legacy 'processing' for backward compat (queued)
$videos['processing'] = $videos['queued'];

$sql             = "SELECT count(*) AS v_suspended FROM video WHERE active='0';";
$rs              = $conn->execute($sql);
