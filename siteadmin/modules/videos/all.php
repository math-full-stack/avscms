    ...
    foreach ( $_POST as $key => $value ) {
        if ( $key != 'check_all_videos' && substr($key, 0, 18) == 'video_id_checkbox_') {        
            if ( $value == 'on' ) {
                ...
                $conn->execute($sql);
                if ( $act == 0 ) {
                    // Suspend: remove from conversion queue to avoid re-activation after conversion
...
// Enrich with queue status for new distinct statuses (2=Baixando, 3=Na Fila/Convertendo)
if ($videos) {
    foreach ($videos as $idx => $v) {
        $queue_status = null;
        $queue_detail = '';
        if ($v['active'] == '3' || $v['active'] == '2') {
            // Check FP queue first
            $qrs = $conn->execute("SELECT status FROM conversion_queue_fp WHERE VID = ".intval($v['VID'])." LIMIT 1");
            if ($conn->Affected_Rows() == 1) {
                $queue_status = $qrs->fields['status'];
                $queue_detail = ($queue_status == '1') ? 'Convertendo (1ª passagem)' : 'Na Fila (1ª passagem)';
            } else {
                $qrs2 = $conn->execute("SELECT status FROM conversion_queue_sp WHERE VID = ".intval($v['VID'])." LIMIT 1");
                if ($conn->Affected_Rows() == 1) {
...
function constructQuery($module)
{
    ...
    if ( isset($_POST['search_videos']) ) {
		...
		if ( $option['active'] == '0' || $option['active'] == '1' || $option['active'] == '2' || $option['active'] == '3' ) {
