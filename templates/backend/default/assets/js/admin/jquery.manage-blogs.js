...
$(document).ready(function(){
    ...
    $("body").on('click', "a[id='view_del_blog']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_delete_blog', { blog_id: blog_id },
			function (response) {
				if (response.status) {
					Messenger().post({
    ...
    $("body").on('click', "a[id*='status_blog_']", function(event) {
		...
		if (processing == 0) {
			...
			var blog_id = split[2];
			$('#' + id).html('<i class="small-loader"></i>');
			$.post(base_url + '/ajax.php?module=admin_status_blog', { blog_id: blog_id, blog_status: blog_status},
				function (response) {
					if (response.status) {
						if (blog_status == 0) {
							Messenger().post({
    ...
    $("body").on('click', "a[id*='view_blog_']", function(event) {
		...
		var split = id.split('_');
		var blog_id = split[2];		
		$.post(base_url + '/ajax.php?module=admin_get_blog', { blog_id: blog_id },
			function (response) {
				if (response.status) {
					//Load Blog Data
	...
	$("body").on('click', "button[id='edit-save']", function(event) {		
		...
		if (!hasErrors("input[id*='edit-']") && !hasErrors("textarea[id*='edit-']")) {
			...
			$.post(base_url + '/ajax.php?module=admin_save_blog', { data: blogData },
				function (response) {					
