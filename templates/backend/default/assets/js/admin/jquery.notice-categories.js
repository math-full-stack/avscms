...
function validateName(input) {
	$(input).change(function(){
		if($.trim($(this).val()).length > 0) {
			...
			var id   = $('#edit-id').val();
			var name = $(this).val();	
			$.post(base_url + '/ajax.php?module=admin_check_category', { type: type, id: id, name: name, slug: '' },
				function (response) {
					if (response.status) {
						if(response.name) {
...
$(document).ready(function(){
    ...
    $("body").on('click', "a[id*='delete_category_']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_delete_category', { category_type: category_type, category_id: category_id },
			function (response) {
				if (response.status) {
					Messenger().post({
	...
	$("body").on('click', "button[id='edit-save']", function(event) {		
		...
		if (!hasErrors("input[id*='edit-']")) {
			...
			$.post(base_url + '/ajax.php?module=admin_save_category', { data: categoryData },
				function (response) {					
    ...
    $("body").on('click', "a[id*='status_category_']", function(event) {
		...
		if (processing == 0) {
			...
			$('#' + id).html('<i class="small-loader"></i>');
			$.post(base_url + '/ajax.php?module=admin_status_category', { category_id: category_id, category_status: category_status},
				function (response) {
					if (response.status) {
