...
$(document).ready(function(){
    ...
    $("body").on('click', "a[id='view_del_notice']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_delete_notice', { notice_id: notice_id },
			function (response) {
				if (response.status) {
					Messenger().post({
    ...
    $("body").on('click', "a[id*='status_notice_']", function(event) {
		...
		if (processing == 0) {
			...
			var notice_id = split[2];
			$('#' + id).html('<i class="small-loader"></i>');
			$.post(base_url + '/ajax.php?module=admin_status_notice', { notice_id: notice_id, notice_status: notice_status},
				function (response) {
					if (response.status) {
						if (notice_status == 0) {
							Messenger().post({
    ...
    $("body").on('click', "a[id*='view_notice_']", function(event) {
		...
		var split = id.split('_');
		var notice_id = split[2];		
		$.post(base_url + '/ajax.php?module=admin_get_notice', { notice_id: notice_id, editor: false },
			function (response) {
				if (response.status) {
					//Load Notice Data
    ...
    $("body").on('click', "a[id*='edit_notice_']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_get_notice', { notice_id: notice_id, editor: true },
			function (response) {
				if (response.status) {
					//Reset Errors
	...
	$("body").on('click', "button[id='edit-save']", function(event) {		
		...
		if (!hasErrors("input[id*='edit-']") && !hasErrors("textarea[id*='edit-']")) {
			...
			$.post(base_url + '/ajax.php?module=admin_save_notice', { data: noticeData },
				function (response) {					
