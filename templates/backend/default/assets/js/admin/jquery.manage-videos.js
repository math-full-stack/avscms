...
// ----- Carousel de capas na listagem (navegacao por setas/dots, sem autoplay) -----

function thumbCarouselNav(carousel, toIndex) {
	var slides = carousel.find('.tc-slide');
	var dots = carousel.find('.tc-dot');
	var n = slides.length;
	if (n === 0) {
		return;
	}
	toIndex = ((parseInt(toIndex, 10) % n) + n) % n;
	slides.removeClass('active').eq(toIndex).addClass('active');
	dots.removeClass('active').eq(toIndex).addClass('active');
}

function thumbCarouselRender(video_id, frames, urls) {
	var box = $('#thumb-carousel-' + video_id);
	if (!box.length) {
		return;
	}
	var d = new Date();
	...
	for (var k = 0; k < frames.length; k++) {
		html += '<img'
			+ (k === 0 ? ' id="thumb-' + video_id + '"' : '')
			+ ' src="' + urls[k] + '?' + d.getTime() + '"'
			+ ' class="img-responsive tc-slide' + (k === 0 ? ' active' : '') + '"'
			+ ' data-frame="' + frames[k] + '"'
			+ ' alt="" />';
		dots += '<span class="tc-dot' + (k === 0 ? ' active' : '') + '"></span>';
	}
	box.find('.tc-link').html(html);
	box.find('.tc-dots').html(dots);
	if (frames.length > 1) {
		box.find('.tc-arrow').show();
		box.find('.tc-dots').show();
	} else {
		box.find('.tc-arrow').hide();
		box.find('.tc-dots').hide();
	}
}
...
$(document).ready(function(){
    ...
    $("body").on('click', "a[id='view_del_video']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_delete_video', { video_id: video_id },
			function (response) {
				if (response.status) {
					Messenger().post({
    ...
    $("body").on('click', "a[id*='unflag_video_']", function(event) {
		...
		$('#' + id).html('<i class="small-loader"></i>');
		$.post(base_url + '/ajax.php?module=admin_unflag_video', { f_id: f_id },
			function (response) {
				if (response.status) {
					Messenger().post({
    ...
    $("body").on('click', "a[id*='thumb_video_']", function(event) {
		...
		if (processing == 0) {
			...
			$.post(base_url + '/ajax.php?module=admin_thumb_video', { video_id: video_id },
				function (response) {
					if (response.status) {
						Messenger().post({
    ...
    $("body").on('click', "a[id*='duration_video_']", function(event) {
		...
		if (processing == 0) {
			...
			var video_id = split[2];
			$('#' + id).html('<i class="small-loader"></i>');
			$.post(base_url + '/ajax.php?module=admin_duration_video', { video_id: video_id },
				function (response) {
					if (response.status) {
						Messenger().post({
    ...
    $("body").on('click', "a[id*='status_video_']", function(event) {
		...
		if (processing == 0) {
			...
			var video_id = split[2];
			$('#' + id).html('<i class="small-loader"></i>');
			$.post(base_url + '/ajax.php?module=admin_status_video', { video_id: video_id, video_status: video_status},
				function (response) {
					if (response.status) {
						if (video_status == 0) {
							Messenger().post({
							...
							$('#status_video_' + video_id).attr('alt', 'Suspender');
							$('#status_video_' + video_id).attr('title', 'Suspender');
							$('#status_video_' + video_id).html('<i class="fa fa-times"></i>');
							$('#status-' + video_id).html('<span class="text-success" alt="Ativo" title="Ativo"><i class="fa fa-check-circle"></i> Ativo</span>');							
						} else {
							// Suspend / Cancel (works for 1,2,3)
							var msg = (video_status == '2') ? 'Download cancelado!' : (video_status == '3' ? 'Removido da fila!' : 'Successfully suspended!');
							Messenger().post({
								message: 'Video <b>ID ' + video_id + '</b>: ' + msg,
								type: 'success'
							});
							$('#status_video_' + video_id).attr('data-status', 0);
							$('#status_video_' + video_id).attr('alt', 'Ativar');
							$('#status_video_' + video_id).attr('title', 'Ativar');
							$('#status_video_' + video_id).html('<i class="fa fa-check"></i>');
							$('#status-' + video_id).html('<span class="text-danger" alt="Inativo" title="Inativo"><i class="fa fa-times-circle"></i> Inativo</span>');
						}
						...
						if (video_status == 0) {
							...
							if (video_status == '2') {
								$('#status_video_' + video_id).html('<i class="fas fa-download"></i>');
							} else if (video_status == '3') {
								$('#status_video_' + video_id).html('<i class="fas fa-hourglass-half"></i>');
							} else {
								$('#status_video_' + video_id).html('<i class="fa fa-times"></i>');
							}							
						}
	...
	//Reprocess Video
    $("body").on('click', "a[id*='reprocess_video_']", function(event) {
        event.preventDefault();
		var processing = $(this).attr('data-processing');
		if (processing == 0) {
			$(this).attr('data-processing', 1);
			...
			var video_id = split[2];
			$('#' + id).html('<i class="small-loader"></i>');
			$.post(base_url + '/ajax.php?module=admin_reprocess_video', { video_id: video_id },
				function (response) {
					if (response.status) {
						Messenger().post({
							message: response.message || 'Video <b>ID ' + video_id + '</b>: Reprocessamento iniciado!',
							type: 'success'
						});
						$('#status_video_' + video_id).attr('data-status', 2);
						$('#status_video_' + video_id).attr('alt', 'Baixando');
						$('#status_video_' + video_id).attr('title', 'Baixando - aguarde');
						$('#status_video_' + video_id).html('<i class="fas fa-download"></i>');
						$('#status-' + video_id).html('<span class="text-warning" alt="Baixando" title="Baixando"><i class="fas fa-download"></i> Baixando</span>');
					} else {
						Messenger().post({
							message: response.error || 'Video <b>ID ' + video_id + '</b>: Falha ao reprocessar!',
							type: 'error'
						});
					}
					$('#' + id).html('<i class="fa fa-refresh"></i>');
					$('#reprocess_video_' + video_id).attr('data-processing', 0);
			}, "json");
		}
	});	
    ...
    $("body").on('click', "a[id*='view_video_']", function(event) {
		...
		var split = id.split('_');
		var video_id = split[2];		
		$.post(base_url + '/ajax.php?module=admin_get_video', { video_id: video_id },
			function (response) {
				if (response.status) {
					//Load Video Data
	...
	$("body").on('click', "button[id='edit-save']", function(event) {		
		...
		if (!hasErrors("input[id*='edit-']") && !hasErrors("textarea[id*='edit-']")) {
			...
			$.post(base_url + '/ajax.php?module=admin_save_video', { data: videoData },
				function (response) {					
					...
					if (response.status) {
						...
						if (videoData.active == 1) {
							...
							$('#status_video_' + video_id).html('<i class="fa fa-times"></i>');
							$('#status-' + video_id).html('<span class="text-success" alt="Ativo" title="Ativo"><i class="fa fa-check-circle"></i> Ativo</span>');							
						} else if (videoData.active == 2) {
							$('#status_video_' + video_id).attr('data-status', 2);
							$('#status_video_' + video_id).attr('alt', 'Baixando');
							$('#status_video_' + video_id).attr('title', 'Baixando');
							$('#status_video_' + video_id).html('<i class="fas fa-download"></i>');
							$('#status-' + video_id).html('<span class="text-warning" alt="Baixando" title="Baixando"><i class="fas fa-download"></i> Baixando</span>');
						} else if (videoData.active == 3) {
							$('#status_video_' + video_id).attr('data-status', 3);
							$('#status_video_' + video_id).attr('alt', 'Na Fila');
							$('#status_video_' + video_id).attr('title', 'Na Fila');
							$('#status_video_' + video_id).html('<i class="fas fa-hourglass-half"></i>');
							$('#status-' + video_id).html('<span class="text-info" alt="Na Fila" title="Na Fila"><i class="fas fa-circle-notch fa-spin"></i> Na Fila</span>');
						} else {
							$('#status_video_' + video_id).attr('data-status', 0);
							$('#status_video_' + video_id).attr('alt', 'Ativar');
							$('#status_video_' + video_id).attr('title', 'Ativar');
    ...
    $("body").on('click', "a[id*='thumbadv_video_']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_get_thumbs_video', { video_id: video_id },
			function (response) {	
	...
	$("body").on('click', "button[id*='thumbnails-regen-']", function(event) {		
		...
		if ($('#thumbnails-processing').val() == '0') {
			...
			$.post(base_url + '/ajax.php?module=admin_thumbnails_video', { video_id: video_id, target: target, black_bars: black_bars, keep_ar: keep_ar },
				function (response) {	
					$('#thumbnails-container-rel').html('');				
					if (response.status && response.count > 0) {
						$('#thumbnails-id-span').html(video_id);
		...
		$.post(base_url + '/ajax.php?module=admin_save_thumbnails', { video_id: video_id, thumbnails_default: thumbnails_default, thumbnails_opt: thumbnails_opt },
			function (response) {					
				...
				if (response.status) {						
					...
					$('#thumb-' + video_id).attr('src', response.src + '?' + d.getTime());
					if (response.frames) {
						thumbCarouselRender(video_id, response.frames, response.urls);
					}
				} else {
					Messenger().post({
	...
	//Carousel de capas na listagem: navegacao por setas/dots (sem autoplay)
	$("body").on('click', '.tc-arrow.tc-prev', function(event) {
		event.preventDefault();
		var box = $(this).closest('.thumb-carousel');
		thumbCarouselNav(box, box.find('.tc-slide.active').index('.tc-slide') - 1);
	});

	$("body").on('click', '.tc-arrow.tc-next', function(event) {
		event.preventDefault();
		var box = $(this).closest('.thumb-carousel');
		thumbCarouselNav(box, box.find('.tc-slide.active').index('.tc-slide') + 1);
	});

	$("body").on('click', '.tc-dot', function(event) {
		event.preventDefault();
		thumbCarouselNav($(this).closest('.thumb-carousel'), $(this).index());
	});
	
	//Close Thumbnails Modal
	$('#thumbnailsModal').on('hidden.bs.modal', function () {
		var video_id = $('#thumbnails-id').val();
		$.post(base_url + '/ajax.php?module=admin_delete_tmp', { video_id: video_id},
			function (response) {
		}, "json");			
	})	
