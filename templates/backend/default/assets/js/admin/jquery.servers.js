...
$(document).ready(function(){
    ...
    $("body").on('click', "a[id*='delete_server_']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_delete_server', { server_id: server_id },
			function (response) {
				if (response.status) {
					Messenger().post({
    ...
    $("body").on('click', "a[id*='status_server_']", function(event) {
		...
		if (processing == 0) {
			...
			var server_id = split[2];
			$('#' + id).html('<i class="small-loader"></i>');
			$.post(base_url + '/ajax.php?module=admin_status_server', { server_id: server_id, server_status: server_status},
				function (response) {
					if (response.status) {
						if (server_status == 0) {
							Messenger().post({
    ...
    $("body").on('click', "a[id*='edit_server_']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_get_server', { server_id: server_id },
			function (response) {
				if (response.status) {
					//Reset Errors
	...
	$("body").on('click', "button[id='edit-save']", function(event) {		
		...
		if (!hasErrors("input[id*='edit-']") && !hasErrors("textarea[id*='edit-']")) {
			...
			$.post(base_url + '/ajax.php?module=admin_save_server', { data: serverData },
				function (response) {					
    ...
    $("body").on('click', "a[id*='used_server_']", function(event) {
		...
		if (processing == 0) {
			...
			var server_id = split[2];
			$('#' + id).html('<i class="small-loader"></i>');
			$.post(base_url + '/ajax.php?module=admin_used_server', { server_id: server_id, server_used: server_used},
				function (response) {
					if (response.status) {
						if (server_used == 0) {
							Messenger().post({
