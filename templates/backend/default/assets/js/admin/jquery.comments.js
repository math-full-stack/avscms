...
$(document).ready(function(){
    ...
    $("body").on('click', "a[id*='comments_']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_get_comments', { id: item_id, type: item_type },
			function (response) {
				if (response.status) {
					//Reset Errors
    ...
    $("body").on('click', "a[id*='delete_comment_']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=admin_delete_comment', { id: item_id, type: item_type },
			function (response) {
				if (response.status) {
					if (response.aid) {
    ...
    $("body").on('click', "button[id*='comment_save_']", function(event) {
		...
		if (!hasErrors('#editval_comment_' + item_type + '_' + item_id)) {
			...
			$.post(base_url + '/ajax.php?module=admin_save_comment', { data: commentData },
				function (response) {
					if (response.status) {
						$('#item_comment_' + item_id).attr('data-status', 'display');					
