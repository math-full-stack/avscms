...
<script type="text/javascript">
	...
	function showToast(message, type) {
		type = type || 'info';
		if (window.Messenger) {
			if (!Messenger.options) Messenger.options = {};
			if (!Messenger.options.extraClasses) Messenger.options.extraClasses = 'messenger-fixed messenger-on-bottom messenger-on-right';
			Messenger().post({ message: message, type: type, hideAfter: 5, showCloseButton: true });
			return;
		}
		if (!document.getElementById('toast-fallback-wrap')) {
			var css = document.createElement('style');
			css.type = 'text/css';
			css.id = 'toast-fallback-css';
			css.innerHTML = '#toast-fallback-wrap{position:fixed;top:20px;right:20px;z-index:99999;max-width:360px;}' +
				'#toast-fallback-wrap .tfb-toast{background:#333;color:#fff;padding:12px 18px;margin-bottom:8px;border-radius:4px;box-shadow:0 2px 8px rgba(0,0,0,.35);cursor:pointer;font-size:13px;word-wrap:break-word;}' +
				'#toast-fallback-wrap .tfb-toast.tfb-success{background:#17884d;}' +
				'#toast-fallback-wrap .tfb-toast.tfb-error{background:#c0392b;}' +
				'#toast-fallback-wrap .tfb-toast.tfb-info{background:#2c6fbb;}' +
				'#toast-fallback-wrap .tfb-toast.tfb-warning{background:#b9770e;}';
			document.head.appendChild(css);
			var wrap = document.createElement('div');
			wrap.id = 'toast-fallback-wrap';
			document.body.appendChild(wrap);
		}
		var el = document.createElement('div');
		el.className = 'tfb-toast tfb-' + type;
		el.textContent = message;
		el.onclick = function() { el.remove(); };
		document.getElementById('toast-fallback-wrap').appendChild(el);
