											...
											<div class="filter col-xs-12 col-sm-6 col-md-4">
												<div class="form-group">
													<select id="filter_active" name="active" style="width:100%">
														...
														<option value="1"{if $option.active == '1'} selected="selected"{/if}>Ativo</option>
														<option value="0"{if $option.active == '0'} selected="selected"{/if}>Inativo</option>
														<option value="2"{if $option.active == '2'} selected="selected"{/if}>Baixando</option>
														<option value="3"{if $option.active == '3'} selected="selected"{/if}>Na Fila / Convertendo</option>
													</select>												
												</div>
											</div>
											<div class="filter col-xs-12 col-sm-6 col-md-4">
										...
										{section name=i loop=$videos}
											<div id="item-{$videos[i].VID}" class="item-main-container">
															...
															<div class="col-xs-6 col-sm-4 col-md-3 col-lg-2">
																...
																<span id="status-{$videos[i].VID}">
																	{if $videos[i].active == 1}
																		<span class="text-success" alt="Ativo" title="Ativo"><i class="fa fa-check-circle"></i> Ativo</span>
																	{elseif $videos[i].active == 2}
																		<span class="text-warning" alt="Baixando" title="Baixando do YouTube/Grabber"><i class="fas fa-download"></i> Baixando</span>
																	{elseif $videos[i].active == 3}
																		{if $videos[i].queue_status == '1'}
																			<span class="text-info" alt="Convertendo" title="{$videos[i].queue_detail}"><i class="fas fa-circle-notch fa-spin"></i> Convertendo</span>
																		{else}
																			<span class="text-primary" alt="Na Fila" title="{$videos[i].queue_detail}"><i class="fas fa-hourglass-half"></i> Na Fila</span>
																		{/if}
																	{else}
																		<span class="text-danger" alt="Inativo" title="Inativo"><i class="fa fa-times-circle"></i> Inativo</span>
																	{/if}
																</span>
																{if $videos[i].queue_detail && $videos[i].active == 3}
																<br><small class="text-muted">{$videos[i].queue_detail}</small>
																{/if}
															</div>
															<div class="col-xs-6 col-sm-4 col-md-3 col-lg-2">
																<div class="d-label">
												...
												<div class="item-actions">																									
													<div class="btn-group">
														{if $videos[i].source_url != ''}
														<a id="reprocess_video_{$videos[i].VID}" class="btn btn-warning" href="#" alt="Reprocessar" title="Reprocessar vídeo a partir da URL de origem" data-processing="0"><i class="fa fa-refresh"></i></a>
														{/if}
														<div class="btn-group">
														...
														{if $videos[i].active == '1'}
															<a id="status_video_{$videos[i].VID}" class="btn btn-success" href="#" alt="Suspender" title="Suspender" data-processing="0" data-status="1"><i class="fa fa-times"></i></a>
														{elseif $videos[i].active == '2'}
															<a id="status_video_{$videos[i].VID}" class="btn btn-warning" href="#" alt="Baixando" title="Baixando - aguarde" data-processing="0" data-status="2"><i class="fas fa-download"></i></a>
														{elseif $videos[i].active == '3'}
															<a id="status_video_{$videos[i].VID}" class="btn btn-info" href="#" alt="Na Fila" title="{$videos[i].queue_detail}" data-processing="0" data-status="3"><i class="fas fa-hourglass-half"></i></a>
														{else}
															<a id="status_video_{$videos[i].VID}" class="btn btn-success" href="#" alt="Ativar" title="Ativar" data-processing="0" data-status="0"><i class="fa fa-check"></i></a>
														{/if}																	
													</div>
