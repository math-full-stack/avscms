...
function mgLoadJobs(status) {
    ...
    mgAjaxGet(url, function(err,data) {
        ...
        if(j.status==='PROCESSING') html+='<button class="btn btn-xs btn-info" onclick="mgRestartJob('+j.id+')" title="Restart"><i class="fa fa-refresh"></i></button> <button class="btn btn-xs btn-warning" onclick="mgPauseJob('+j.id+')" title="Pause"><i class="fa fa-pause"></i></button> <button class="btn btn-xs btn-danger" onclick="mgCancelJob('+j.id+')" title="Cancel"><i class="fa fa-times"></i></button> ';
...
function mgRestartJob(id) { var fd=new FormData(); fd.append('id',id); mgAjax('videos.php?m=mass_grabber&a=restart_job',fd,function(e,d){ if(d&&d.message) showToast(d.message, 'info'); mgLoadJobs(''); }); }
