...
function doSignupFBModal(response) {
	...
	$.post(base_url + '/ajax.php?module=fb_signup', { email: response.email, fname: response.first_name, lname: response.last_name, id: response.id },
		function (response) {					
...
$.post(base_url + '/ajax.php?module=signup_check_username', { username: username },
	function (response) {
		if (response.status == 1) {
			if (response.valid == 1) {
