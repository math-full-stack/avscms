$(document).ready(function(){            
	...
	function doneTyping () {
		...
		$('#cvideos-loader').show();		
		if (keyword != '') {
			$.post(base_url + '/ajax.php?module=comments_get_svideos', {page: 1, keyword: keyword}, 
			function (response) {
			...
			$.post(base_url + '/ajax.php?module=comments_get_fvideos', {page: 1},
			function (response) {
				if (response.status == '1') {
					$('#cvideos-container').html(response.code);					
					var $images = $('#cvideos-container img');
	...
	function doneTypingP () {
		...
		$('#cphotos-loader').show();		
		if (keyword != '') {
			$.post(base_url + '/ajax.php?module=comments_get_sphotos', {page: 1, keyword: keyword}, 
			function (response) {
				if (response.status == '1') {
			...
			$.post(base_url + '/ajax.php?module=comments_get_fphotos', {page: 1},
			function (response) {
				if (response.status == '1') {
					$('#cphotos-container').html(response.code);					
					var $images = $('#cphotos-container img');
	...
	$("body").on('click', "[id*='comments_favorite_videos_']", function(event) {				
		...
		$('#cvideos-container').hide();
		$('#cvideos-loader').show();
		$.post(base_url + '/ajax.php?module=comments_get_fvideos', {page: page}, 
		function (response) {
			if (response.status == '1') {
				$('#cvideos-container').html(response.code);					
				var $images = $('#cvideos-container img');
	...
	$("body").on('click', "[id*='comments_search_videos_']", function(event) {				
		...
		$('#cvideos-container').hide();
		$('#cvideos-loader').show();
		$.post(base_url + '/ajax.php?module=comments_get_svideos', {page: page, keyword:keyword}, 
		function (response) {
			if (response.status == '1') {
				$('#cvideos-container').html(response.code);					
				var $images = $('#cvideos-container img');
	...
	$("body").on('click', "[id*='comments_favorite_photos_']", function(event) {				
		...
		$('#cphotos-container').hide();
		$('#cphotos-loader').show();
		$.post(base_url + '/ajax.php?module=comments_get_fphotos', {page: page}, 
		function (response) {
			if (response.status == '1') {
				$('#cphotos-container').html(response.code);					
				var $images = $('#cphotos-container img');
	...
	$("body").on('click', "[id*='comments_search_photos_']", function(event) {				
		...
		$('#cphotos-container').hide();
		$('#cphotos-loader').show();
		$.post(base_url + '/ajax.php?module=comments_get_sphotos', {page: page, keyword:keyword}, 
		function (response) {
			if (response.status == '1') {
				$('#cphotos-container').html(response.code);					
				var $images = $('#cphotos-container img');
	...
	$("body").on('click', "[id='post_comment']", function(event) {	
        ...
        if ( comment != '' ) {
			...
			$.post(base_url + '/ajax.php?module=post_comment', { id: id, type: type, comment: comment },
			function(response) {
	...
	$("body").on('click', "[id='comments_show_more']", function(event) {	
        ...
        if ( page ) {
			...
			$.post(base_url + '/ajax.php?module=load_comments', { id: id, type: type, page: page, order: order},
			function(response) {
				if ( response.status == '0' ) {
				} else {
	...
	$("body").on('click', "[id*='replies_show_more_']", function(event) {	
        ...
        if ( page ) {
			...
			$.post(base_url + '/ajax.php?module=load_replies', { id: id, type: type, page: page, order: order },
			function(response) {
				if ( response.status == '0' ) {
				} else {
	...
	$("body").on('click', "[id*='replies_hide_']", function(event) {	
        ...
        var type 	=  $(this).attr('data-type');
		$(this).hide();
		$.post(base_url + '/ajax.php?module=total_replies', { id: id, type : type },
		function(response) {
			if (response.total > 0) {
	...
	$("body").on('click', "[data-delete-comment]", function(event) {				
		...
		$.post(base_url + '/ajax.php?module=delete_comment', { cid: cid, type : type },
		function(response) {		
			if ( response.status == '0' ) {
				alertBottom(response.msg, 'error');
			} else {
	...
	$("body").on('click', "[id*='report_comment_']", function(event) {				
		...
		$.post(base_url + '/ajax.php?module=report_comment', {  type : type, comment_id: cid, parent_id: pid },
		function(response) {		
			if ( response.status == '0' ) {
				alertBottom(response.msg, 'error');			
			} else {
	...
	$("body").on('click', "[id*='post_reply_']", function(event) {	
        ...
        if ( comment != '' ) {		
			...
			$("#" + response_id).show();
			$.post(base_url + '/ajax.php?module=post_reply', { id: id, type: type, comment: comment },
			function(response) {
				if ( response.status == '0' ) {
					$("#" + response_id).hide();					
					alertBottom(response.msg, 'error');						
	...
	$("body").on('click', "[id*='comment_vote_']", function(event) {	
        ...
        var type      	= id_split[3];		
        var id        	= id_split[4];
		$.post(base_url + '/ajax.php?module=vote_comment', { id: id, type: type, vote: vote },
		function(response) {
			if ( response.status == '0' ) {
				alertBottom(response.msg, 'error');
			} else {								
