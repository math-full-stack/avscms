$(document).ready(function(){
    ...
    $("body").on('click', "a[id='user_subscription']", function(event) {
        ...
        $.post(base_url + '/ajax.php?module=user_subscription', { user_id: user_id },
        function(response) {
    ...
    $("body").on('click', "a[id='user_friendship']", function(event) {
		...
		if (friendship == 'Add') {
			$.post(base_url + '/ajax.php?module=user_friendship', { user_id: user_id, friendship: friendship },
			function(response) {
				if ( response.status == 0 ) {
					alertBottom(response.msg, 'error');			
				} else {
    ...
    $("body").on('click', "a[id='user_block']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=user_block', { user_id: user_id, action: action },
		function(response) {
			if ( response.status == 0 ) {
				alertBottom(response.msg, 'error');			
			} else {
    ...
    $("body").on('click', "button[id='submit_user_report']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=user_report', { user_id: user_id, reason: reason, other: other },
		function(response) {
			if ( response.status == 0 ) {
				alertBottom(response.msg, 'error');			
			} else {
    ...
    $("body").on('click', "button[id='send_user_message']", function(event) {
		...
		$.post(base_url + '/ajax.php?module=user_message', { receiver: receiver, sender: sender, message_subject: message_subject, message_body: message_body },
		function(response) {
			if ( response.status == 0 ) {
				alertBottom(response.msg, 'error');			
			} else {
    ...
    $("body").on('click', "button[id*='confirm_delete_video_']", function(event) {
        event.preventDefault();
        var video_id    = $(this).attr('data-vid');
		$.post(base_url + '/ajax.php?module=delete_video', { video_id: video_id },
		function(response) {
    ...
    $("body").on('click', "button[id*='confirm_delete_album_']", function(event) {
        event.preventDefault();
        var album_id    = $(this).attr('data-aid');
		$.post(base_url + '/ajax.php?module=delete_album', { album_id: album_id },
		function(response) {
			$("#dialogModal").modal('hide');
			if ( response.status == 0 ) {
				alertBottom(response.msg, 'error');			
    ...
    $("body").on('click', "button[id*='confirm_remove_favorite_video_']", function(event) {
        event.preventDefault();
        var video_id    = $(this).attr('data-vid');
		$.post(base_url + '/ajax.php?module=remove_video_favorite', { video_id: video_id },
		function(response) {
			$("#dialogModal").modal('hide');
			if ( response.status == 0 ) {
				alertBottom(response.msg, 'error');			
    ...
    $("body").on('click', "button[id*='confirm_remove_favorite_photo_']", function(event) {
        event.preventDefault();
        var photo_id    = $(this).attr('data-pid');
		$.post(base_url + '/ajax.php?module=remove_photo_favorite', { photo_id: photo_id },
		function(response) {
			$("#dialogModal").modal('hide');
			if ( response.status == 0 ) {
				alertBottom(response.msg, 'error');			
    ...
    $("body").on('click', "button[id*='confirm_delete_blog_']", function(event) {
        event.preventDefault();
        var blog_id    = $(this).attr('data-bid');
		$.post(base_url + '/ajax.php?module=delete_blog', { blog_id: blog_id },
		function(response) {
			$("#dialogModal").modal('hide');
			if ( response.status == 0 ) {
				alertBottom(response.msg, 'error');			
    ...
    $("body").on('click', "button[id*='confirm_remove_playlist_video_']", function(event) {
        event.preventDefault();
        var video_id    = $(this).attr('data-vid');
		$.post(base_url + '/ajax.php?module=remove_video_playlist', { video_id: video_id },
		function(response) {
			$("#dialogModal").modal('hide');
			if ( response.status == 0 ) {
				alertBottom(response.msg, 'error');			
    ...
    $("body").on('click', "a[id*='block_username_'],a[id*='unblock_username_']", function(event) {  	
        ...
        $.post(base_url + '/ajax.php?module=' + action + '_user', { user_id: user_id },
        function(response) {
