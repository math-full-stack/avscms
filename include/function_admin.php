...
function deleteVideo( $vid )
{
    ...
    // Clean conversion queue entries if any (was processing)
...
function insert_comment_count( $options )
{
    ...
    $uid        = intval($options['UID']);    
    // wall table was renamed to wall_comments (CID = wall_id, WID = OID)
    $sql        = "SELECT COUNT(CID) AS total_comments FROM wall_comments WHERE WID = " .$uid;
    $rs         = $conn->execute($sql);
...
function deleteUser( $uid )
{
    ...
    $tables = array('blog', 'signup', 'users_prefs', 'blog_comments', 'notice_comments', 'photo_comments', 'video_comments', 
					'wall_comments', 'confirm', 'favourite', 'friends', 'notice', 'photo_favorites', 
    ...
    // also delete wall comments where WID = deleted user (wall owner)
    $sql = "DELETE FROM wall_comments WHERE WID = " .$conn->qStr($uid). "";
    $conn->execute($sql);
...
function delete_user_comment($cid) {
	...
	global $conn;
		...
		$sql = "DELETE FROM wall_comments WHERE CID = " .$conn->qStr($cid). " LIMIT 1";
		$conn->execute($sql);
        if ( $conn->Affected_Rows() ) {	
			return true;		
        } else {
